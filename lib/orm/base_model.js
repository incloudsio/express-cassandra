...
BaseModel._sync_es_index = function f(callback) {
  ...
  const getFallbackField = () => {
    ...
    const partitionFields = Array.isArray(partitionKey) ? partitionKey.map(item => String(item)) : partitionKey ? [String(partitionKey)] : [];
  ...
  const ensureSecondaryIndex = next => {
    ...
    const indexClasses = ['org.elassandra.index.ExtendedElasticSecondaryIndex', 'org.elassandra.index.ElasticSecondaryIndex'];
    const fallbackField = getFallbackField();
    const runCreate = (indexClass, targetField, done) => {
      const query = util.format('CREATE CUSTOM INDEX IF NOT EXISTS "%s" ON "%s" ("%s") USING \'%s\';', secondaryIndexName, tableName, targetField, indexClass);
      this._driver.execute_definition_query(query, done);
    };
    const attempts = [];
    for (let i = 0; i < indexClasses.length; i += 1) {
      attempts.push({
        indexClass: indexClasses[i],
        targetField: '_id'
      });
...
BaseModel.get_find_query = function f(queryObject, options) {
  ...
  let query = util.format('SELECT %s%s FROM "%s"', options.distinct ? 'DISTINCT ' : '', selectClause, options.materialized_view ? options.materialized_view : this._properties.table_name);
...
BaseModel.search = function f(queryObject, callback) {
  ...
  // Drop the deprecated `type` parameter. The opensearch client (and modern
  // Elasticsearch) rejects it; on legacy ES/Elassandra the table maps 1:1 to a
  // single type per index so it's safe to omit.
  ...
  const handleResult = (err, response) => {
    ...
    // opensearch-js wraps responses in `{ body, statusCode, ... }`. Unwrap so
    // callers that expect the legacy client's raw response keep working.
  ...
  if (esClient && esClient.__kind === 'opensearch') {
    const p = esClient.search(query);
    if (p && typeof p.then === 'function') {
      p.then(res => handleResult(null, res), err => handleResult(err));
    } else {
      handleResult(null, p);
    }
    return;
  ...
  esClient.search(query, handleResult);
};
...
BaseModel.update = function f(queryObject, updateValues, options, callback) {
  ...
  const { updateClauses, queryParams, errorHappened } = parser.get_update_value_expression(this, schema, updateValues, callback);
...
BaseModel.prototype.save = function fn(options, callback) {
  ...
  let query = util.format('INSERT INTO "%s" ( %s ) VALUES ( %s )', properties.table_name, identifiers.join(' , '), values.join(' , '));
