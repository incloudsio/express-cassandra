...
// Prefer the OpenSearch JS client (matches Elassandra's OpenSearch 1.3
// runtime and modern Elassandra builds). Fall back to the legacy
// `elasticsearch` npm package so existing users stay on their rails.
let esClientFactory = null;
let esClientKind = null;
try {
  // eslint-disable-next-line import/no-extraneous-dependencies, import/no-unresolved, global-require
  const osMod = require('@opensearch-project/opensearch');
  esClientFactory = cfg => new osMod.Client(cfg);
  esClientKind = 'opensearch';
} catch (e) {
  try {
    // eslint-disable-next-line import/no-extraneous-dependencies, import/no-unresolved, global-require
    const esMod = require('elasticsearch');
    esClientFactory = cfg => new esMod.Client(cfg);
    esClientKind = 'legacy';
  } catch (e2) {
...
Apollo.prototype = {
  ...
  create_es_client() {
    if (!esClientFactory) {
      throw new Error('Configured to use elassandra, but no OpenSearch/Elasticsearch client was found. Try: npm install @opensearch-project/opensearch');
    }

    const contactPoints = this._connection.contactPoints;

    // Sniffing is intentionally disabled: the `publish_address` returned by
    // `/_nodes/_all/http` is usually an in-cluster IP that is unreachable
    // from the application host, which caused the 60s request-timeout users
    // hit against modern Elassandra (OpenSearch 1.3) builds.
    let defaultConfig;
    if (esClientKind === 'opensearch') {
      // @opensearch-project/opensearch takes full URL strings as `nodes`.
      defaultConfig = {
        nodes: contactPoints.map(host => `http://${host}:9200`)
      };
    } else {
      defaultConfig = {
        hosts: contactPoints.map(host => ({ host })),
        sniffOnStart: false
      };
    }

    const esClientConfig = _.defaults({}, this._connection.elasticsearch, defaultConfig);
    if (esClientKind === 'opensearch') {
      // Legacy configs often carry `sniffOnStart: true` from elasticsearch-js,
      // which can hang against clustered Elassandra/OpenSearch deployments by
      // resolving to internal publish addresses. Keep sniffing disabled.
      esClientConfig.sniffOnStart = false;
    }
    this._esclient = esClientFactory(esClientConfig);
    // Tag the client so the builder and base_model can branch on
    // callback vs promise API shape without re-detecting.
    this._esclient.__kind = esClientKind;
  ...
  init(callback) {
    const onUserDefinedAggregates = err => {
      ...
      Promise.all(managementTasks).then(() => {
        callback(null, this);
      }).catch(err1 => {
        callback(err1);
      });
  ...
  close(callback) {
    ...
    Promise.all(clientsToShutdown).then(() => {
      callback();
    }).catch(err => {
      callback(err);
    });
