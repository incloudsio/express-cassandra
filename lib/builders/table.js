...
TableBuilder.prototype = {
  ...
  get_table(callback) {
    ...
    this._driver.execute_query(query, [tableName, keyspaceName], (err, resultColumns) => {
      ...
      for (let r = 0; r < resultColumns.rows.length; r++) {
        ...
        if (this._properties.esclient && row.column_name === '_source') {
          // Elassandra stores OpenSearch document source in Cassandra; it is
          // managed by the search layer, not by the user model schema.
          continue;
        }
  ...
  create_table(schema, callback) {
    ...
    const query = util.format('CREATE TABLE IF NOT EXISTS "%s" (%s , PRIMARY KEY((%s)%s))%s;', tableName, rows.join(' , '), clauses.partitionKeyClause, clauses.clusteringKeyClause, clauses.clusteringOrderClause);
  ...
  drop_table(materializedViews, callback) {
    const properties = this._properties;
    const tableName = properties.table_name;
    const message = util.format('Schema for table "%s" has changed in a way where alter migration is not possible, all data in the table will be lost, are you sure you want to drop the table?', tableName);
      ...
      for (let r = 0; r < resultIndexes.rows.length; r++) {
        ...
        if (row.index_name && row.options.target) {
          ...
          if (row.kind === 'CUSTOM') {
            ...
            if (using === 'org.elassandra.index.ElasticSecondaryIndex' || using === 'org.elassandra.index.ExtendedElasticSecondaryIndex') {
              continue;
            }
  ...
  _create_custom_index_query(tableName, customIndex) {
    let query = util.format('CREATE CUSTOM INDEX IF NOT EXISTS ON "%s" ("%s") USING \'%s\'', tableName, customIndex.on, customIndex.using);
  ...
  _create_materialized_view_query(tableName, viewName, viewSchema) {
    ...
    const query = util.format('CREATE MATERIALIZED VIEW IF NOT EXISTS "%s" AS SELECT %s FROM "%s" WHERE %s PRIMARY KEY((%s)%s)%s;', viewName, rows.join(' , '), tableName, whereClause, clauses.partitionKeyClause, clauses.clusteringKeyClause, clauses.clusteringOrderClause);
  ...
  create_mviews(materializedViews, callback) {
    ...
    async.eachSeries(Object.keys(materializedViews), (viewName, next) => {
      const query = this._create_materialized_view_query(tableName, viewName, materializedViews[viewName]);
      this._driver.execute_definition_query(query, (err, result) => {
  ...
  _apply_alter_operations(alterOperations, dbSchema, normalizedModelSchema, normalizedDBSchema, callback) {
    ...
    const addedCustomIndexes = _.filter(normalizedModelSchema.custom_indexes, obj => !_.find(normalizedDBSchema.custom_indexes, obj));
    const removedCustomIndexes = _.filter(normalizedDBSchema.custom_indexes, obj => !_.find(normalizedModelSchema.custom_indexes, obj));
    removedCustomIndexes.forEach(removedIndex => {
      removedIndexNames.push(dbSchema.index_names[objectHash(removedIndex)]);
    });

    const addedMaterializedViewsNames = _.filter(Object.keys(normalizedModelSchema.materialized_views), viewName => !_.isEqual(normalizedDBSchema.materialized_views[viewName], normalizedModelSchema.materialized_views[viewName]));

    const removedMaterializedViewNames = _.filter(Object.keys(normalizedDBSchema.materialized_views), viewName => !_.isEqual(normalizedDBSchema.materialized_views[viewName], normalizedModelSchema.materialized_views[viewName]));
    ...
    if (removedMaterializedViewNames.length > 0) {
      const message = util.format('Schema for table "%s" has removed materialized_views: %j, are you sure you want to drop them?', tableName, removedMaterializedViewNames);
      const permission = this._confirm_migration(message);
      if (permission !== 'y') {
        callback(buildError('model.tablecreation.schemamismatch', tableName, 'migration suspended, please apply the change manually'));
        return;
    ...
    this.drop_mviews(removedMaterializedViewNames, err2 => {
      ...
      if (removedIndexNames.length > 0) {
        const message = util.format('Schema for table "%s" has removed indexes: %j, are you sure you want to drop them?', tableName, removedIndexNames);
        const permission = this._confirm_migration(message);
        if (permission !== 'y') {
          callback(buildError('model.tablecreation.schemamismatch', tableName, 'migration suspended, please apply the change manually'));
          return;
  ...
  init_alter_operations(modelSchema, dbSchema, normalizedModelSchema, normalizedDBSchema, callback) {
    ...
    async.eachSeries(differences, (diff, next) => {
      ...
      if (diff.kind === 'N') {
        const message = util.format('Schema for table "%s" has added field "%s", are you sure you want to alter to add the field?', tableName, fieldName);
      ...
      if (diff.kind === 'D') {
        const message = util.format('Schema for table "%s" has removed field "%s", all data in the field will lost, are you sure you want to alter to drop the field?', tableName, fieldName);
        alterOperations.push({
          fieldName,
          message,
          operation: 'DROP'
      ...
      if (diff.kind === 'E') {
        ...
        if (diff.path[1] === 'type') {
          ...
          if (normalizedDBSchema.key[0].includes(fieldName) || normalizedDBSchema.key.indexOf(fieldName) > 0) {
            ...
            const message = util.format('Schema for table "%s" has new type for field "%s", are you sure you want to alter to update the field type?', tableName, fieldName);
            alterOperations.push({
              fieldName,
              message,
              operation: 'ALTER',
            ...
            const message = util.format('Schema for table "%s" has new type for field "%s", all data in the field will be lost, are you sure you want to drop the field & recreate it?', tableName, fieldName);
            alterOperations.push({
              fieldName,
              message,
              operation: 'DROP'
